...
**注：仍接受来自社区的 pull requests 和 issues**
...
我们注意到**Saint He**的代码中有极为先进的`if`语句，于是我们在10甚至9赛博分钟内加入了这个语句。

此语句的文法为：`IF expr stmt SEMICOLON`。

为了允许if执行多条语句，我们又引入了**块**。现在，你可以以以下方式使用一个块：

```c
{print 15;}
```

在块内的代码**必须加入分号**，并且单独的块会被立即执行。

不能换行，因为原作者懒得写，我也没写，我也懒得写了。

你可以用以下方式体验赛博`if`：

```c
if 1 {print 1;print 5;}
```
